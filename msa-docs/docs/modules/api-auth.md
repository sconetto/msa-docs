...
A configuração da API de Autenticação se dá por meio de variáveis de ambiente do
...
- `KEYCLOAK_ADMIN`: Nome do usuário administrador da API do Keycloak
- `KEYCLOAK_PASSWORD`: Senha do usuário para autenticação na API do Keycloak.
...
deseja para o ambiente), neste exemplo usarei o nome **auth**:
...
Para executar os testes da API Auth siga o roteiro abaixo descrito:

1.  Ative a _virtualenv_ **auth**;
...
pytest --cov=auth src/                     # Executa os testes avaliando a cobertura
pytest --cov=auth --cov-report xml src/    # Gera o relatório em XML da cobertura
...
Para fazer a construção do serviço do API Auth basta seguir o roteiro abaixo:
